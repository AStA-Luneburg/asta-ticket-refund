...
<?php endif; ?>
            ...
            <span class="<?php echo \Illuminate\Support\Arr::toCssClasses(['dark:text-white' => config('tables.dark_mode')]) ?>" x-text="pluralize(<?php
    if (is_object(__('tables::table.selection_indicator.selected_count')) || is_array(__('tables::table.selection_indicator.selected_count'))) {
        echo "JSON.parse(atob('".base64_encode(json_encode(__('tables::table.selection_indicator.selected_count')))."'))";
    } elseif (is_string(__('tables::table.selection_indicator.selected_count'))) {
        echo "'".str_replace("'", "\'", __('tables::table.selection_indicator.selected_count'))."'";
    } else {
        echo json_encode(__('tables::table.selection_indicator.selected_count'));
    }
?>, selectedRecords.length, { count: selectedRecords.length })"></span>
...
<?php /**PATH /Users/mat/Projects/AStA/asta-ticket-refund/vendor/filament/tables/src/../resources/views/components/selection-indicator.blade.php ENDPATH**/ ?>